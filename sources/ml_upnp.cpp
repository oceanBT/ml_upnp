...
//
///*----------------------------------------------------------------------
//|   CWinamp_MediaController::LoadPreferences
//+---------------------------------------------------------------------*/
//NPT_Result CWinamp_MediaController::LoadPreferences(void)
//{
//  if (m_WAConfFile != NULL)
//  {
//    if (NPT_SUCCEEDED(m_ListView.LoadPreferences(CONFIG_SECTION, m_WAConfFile)))
//      return NPT_SUCCESS;
//  }
//  return NPT_FAILURE;
//}
...
//{
//  if (PLT_SyncMediaBrowser::GetMediaServersMap().GetEntryCount() == 0)
//  {
...
//  {
//    AppendMenu(playToMenu, 0, ID_UPNPITEMCONTEXT_PLAYTO_RENDER_NULL, WINAMP_NO_RENDER_DEVICE_FOUND);
//  }
//  else
//  {
...
//
//      NPT_AutoLock lock2(m_MSUUIDs);
//      m_MSUUIDs.Put(NewTree.id, uuid);
//
//      PLT_SyncMediaBrowser::OnDeviceAdded(device);
//    }
//  }
//
//  this->CheckMSDeviceFound();
...
//
//      NPT_AutoLock lock2(m_MSUUIDs);
//      m_MSUUIDs.Erase(Ptr_repository->GetMLId());
//
//      PLT_SyncMediaBrowser::OnDeviceRemoved(device);
//    }
//  }
//
//  this->CheckMSDeviceFound();
...
//{
//  NPT_Cardinal new_value = MENU_MEDIARENDERER_OFFSET + m_MediaRenderers.GetEntryCount() + 1;
//  NPT_String uuid = device->GetUUID();
//
...
//
//    NPT_AutoLock lock2(m_MRUUIDs);
//    m_MRUUIDs.Put(new_value, uuid);
//
//    // Ajout du menu
//    HMENU menu=GetSubMenu(m_PopupMenu,1);
//    HMENU playToMenu=GetSubMenu(menu,3);
...
//{
//  NPT_Cardinal value = 0;
//
//  NPT_String uuid = device->GetUUID();
//  NPT_AutoLock lock(m_MediaRenderers);
//  m_MediaRenderers.Erase(uuid);
//
//  if NPT_SUCCEEDED(this->FindMRIdByUUID(uuid, &value))
//  {
//    // Suppression du menu
//    HMENU menu=GetSubMenu(m_PopupMenu,1);
//    HMENU playToMenu=GetSubMenu(menu,3);
//    DeleteMenu(playToMenu, value, MF_BYCOMMAND);
//  }
//
//  this->CheckMRDeviceFound();
//}
...
//
///*----------------------------------------------------------------------
//|   CWinamp_MediaController::FindMSUUID
//+---------------------------------------------------------------------*/
//NPT_Result
//CWinamp_MediaController::FindMSUUID(NPT_Cardinal id, NPT_String* uuid)
//{
//  NPT_String* uuid_tmp;
//
//  if (m_IsAlive != true)
//    return NPT_FAILURE;
//
//  NPT_AutoLock lock(m_MSUUIDs);
//  if (m_MSUUIDs.Get(id, uuid_tmp) != NPT_SUCCESS)
//    return NPT_FAILURE;
//
...
//
///*----------------------------------------------------------------------
//|   CWinamp_MediaController::FindMRUUID
//+---------------------------------------------------------------------*/
//NPT_Result
//CWinamp_MediaController::FindMRUUID(NPT_Cardinal id, NPT_String* uuid)
//{
//  NPT_String* uuid_tmp;
//
//  if (m_IsAlive != true)
//    return NPT_FAILURE;
//
//  NPT_AutoLock lock(m_MRUUIDs);
//  if (m_MRUUIDs.Get(id, uuid_tmp) != NPT_SUCCESS)
//    return NPT_FAILURE;
//
//  uuid->Erase(0, uuid->GetLength());
//  uuid->Append(uuid_tmp->GetChars());
...
//
///*----------------------------------------------------------------------
//|   CWinamp_MediaController::FindMRIdByUUID
//+---------------------------------------------------------------------*/
//NPT_Result
//CWinamp_MediaController::FindMRIdByUUID(NPT_String uuid, NPT_Cardinal* id)
//{
////  NPT_CHECK_POINTER_FATAL(id);
//
//  if (m_IsAlive != true)
//    return NPT_FAILURE;
//
//  NPT_AutoLock lock(m_MRUUIDs);
//  NPT_List<CWinamp_UUIDsMapEntry*> entries = m_MRUUIDs.GetEntries();
//  if (entries.GetItemCount() != 0) 
//  {
//    NPT_List<CWinamp_UUIDsMapEntry*>::Iterator entry = entries.GetFirstItem();
//    while (entry) 
//    {
//      NPT_String uuid_tmp = (*entry)->GetValue();
//      if (uuid_tmp == uuid)
//      {
//        *id = (*entry)->GetKey();
//        return NPT_SUCCESS;
...
//
///*----------------------------------------------------------------------
//|   CWinamp_MediaController::BrowseDevice
//+---------------------------------------------------------------------*/
//NPT_Result 
...
//
//      /* On efface la liste avant de la mettre à jour */
//      if (!m_MostRecentBrowseResults.IsNull()) 
//          m_MostRecentBrowseResults->Clear();
//
...
//
//      TCHAR tmp[512];
//      sprintf_s(tmp, sizeof(tmp), "There were %d items (%d files)\n", m_MostRecentBrowseResults->GetItemCount(), m_ListView.CountItem());
...
//          return (NPT_Result) CreateDialogParam(PluginUPNP.hDllInstance, MAKEINTRESOURCE(IDD_UPNP_ROOT), (HWND)param2, (DLGPROC)::MainDlgProc, (LPARAM)param1);
//        }
//        else
//        {
...
//+---------------------------------------------------------------------*/
//void CWinamp_MediaController::play(HWND hwnd, INT_PTR enqueue)
//{
...
//+---------------------------------------------------------------------*/
//void CWinamp_MediaController::playTo(HWND hwnd, NPT_String uuid)
//{
//  NPT_String                IdObject;
//  TCHAR                     szBuf1[1024];
...
//  {
//#ifdef UNICODE
//    MessageBox(PluginUPNP.hwndLibraryParent,L"The ml_upnp plug-in don't support multiselection of track",
//								  CA2T(PluginUPNP.description),MB_OK|MB_ICONINFORMATION);
//#else
//    MessageBox(PluginUPNP.hwndLibraryParent,"The ml_upnp plug-in don't support multiselection of track",
//								  PluginUPNP.description,MB_OK|MB_ICONINFORMATION);
//#endif
//    return;
//  }
//
//  NPT_AutoLock lock(m_MediaRenderers);
//  m_MediaRenderers.Get(uuid, device);
//
//  NPT_Cardinal nItem = ListView_GetNextItem(listWnd, -1, LVNI_SELECTED);
//  ListView_GetItemText(listWnd, nItem, 0, szBuf1, sizeof(szBuf1));
//  IdObject.Erase(0, IdObject.GetLength());
//  IdObject.Append(szBuf1);
//
//  PLT_MediaObject* track = NULL;
//  if (NPT_SUCCEEDED(NPT_ContainerFind(*m_MostRecentBrowseResults, PLT_MediaItemIDFinder(IdObject), track))) 
//  {
//    if (track->m_Resources.GetItemCount() > 0) 
//    {
//      NPT_Cardinal resource_index = 0;
//      if (NPT_SUCCEEDED(FindBestResource(*device, *track, resource_index))) 
//      {
...
//      {
//#ifdef UNICODE
//        MessageBox(PluginUPNP.hwndLibraryParent,L"No matching resource found",
//								  CA2T(PluginUPNP.description),MB_OK|MB_ICONINFORMATION);
//#else
//        MessageBox(PluginUPNP.hwndLibraryParent,"No matching resource found",
//								  PluginUPNP.description,MB_OK|MB_ICONINFORMATION);
//#endif
//      }
//    }
...
//  {
//    NPT_Cardinal ret = this->WADlg_handleDialogMsgs(hwnd,uMsg,wParam,lParam);
//    if (ret) return ret;
//  }
//  else
...
//              this->play(hwnd, CWinamp_PLAY);
//              break;
//            case ID_UPNPITEMCONTEXT_ENQUEUE:
//              this->play(hwnd, CWinamp_ENQUEUE);
//              break;
//            default:
//              Res = this->FindMRUUID(r, &uuid);
//              if (Res == NPT_SUCCESS)
//                this->playTo(hwnd, uuid);
//              break;
//          }
//        }
